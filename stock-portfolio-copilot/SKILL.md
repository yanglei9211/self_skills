...
当用户明确提供“当前持仓截图 / 券商持仓页”，并且口头说明“**当前持仓以截图为准**”时，必须按下面规则执行：

- **当前持仓真值 = 用户提供的持仓截图**
- `trade add` / `trade_ledger` 的职责降级为：**记录买点 / 卖点 / 复盘时间线**
- **不要**再把 `trade_ledger` 当作“当前成本价 / 当前持仓数量 / 当前市值”的唯一真值来源
- 如截图与 `seed + trades + sync` 推导结果冲突，**以截图覆盖当前快照**

推荐做法：

- 保留真实成交时间、方向、价格、数量，方便后续分析买卖点
- 另外写入一份 `portfolio_snapshot` 的手工真值快照，`source` 用 `manual_*`
- 后续展示 / 分析 / 汇总默认优先读取 `manual_*` 快照，而不是较新的普通 `sync` 快照

解释口径：

- `trade_ledger`：服务于“为什么买 / 为什么卖 / 何时动手”的复盘
- `portfolio_snapshot`：服务于“现在账户里到底还剩什么、成本多少、浮盈亏多少”的当前状态
- 当两者冲突且用户已给出券商持仓截图时，**当前状态服从截图**
