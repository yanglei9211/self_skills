...
description: >-
  ...
  当用户提到 A股、港股、中概、股票分析、个股分析、全A初筛、候选池、板块扫描、行情、涨幅榜、跌幅榜、
...
$SMH screen --horizon swing --top 15  # A 股 1-4 周中短线初筛
...
> 用户语境："全 A 先帮我筛一批 1-4 周机会"、"找 10-30 个值得深度分析的 A 股候选"、"做一个趋势和筑底反转候选池"

**入口**：`smh screen`

```bash
$SMH screen --horizon swing --top 15
$SMH screen --horizon swing --top 10 --pool-size 800 --format json
```

**定位**：第一阶段候选池生成器，用于把全 A 大中盘 / 活跃股池缩小到 10-30 只，再交给 `smh company CODE --deep` 或 `stock-portfolio-copilot` 自选 / 执行计划复核；它不是直接买入建议。

**两条独立路径**：

- **趋势延续**：强资金流入、站上 MA20/MA60、处在 60 日高位但不过热，输出 `trend_entry_candidate` / `trend_pullback_watch`。
- **筑底反转**：`REVERSAL_INFLOW_CONFIRMED` 或流出衰竭、低位修复、RSI 从低位回升，输出 `reversal_probe_candidate` / `reversal_confirm_watch`。

**硬过滤**：ST / 退市风险名称、成交额过低、换手不足、RSI 极端过热、资金流短长周期冲突、短期涨幅过大且突破高位的追高票。
...
| A 股 1-4 周中短线初筛（趋势延续 / 筑底反转） | ✅ v1.8 已加 |
| 卖方研报观点 | ⏳ v2.5 |
...
用户："全 A 先筛 20 个 1-4 周中短线候选"
→ Agent 调用 `smh screen --horizon swing --top 10`，分别输出趋势延续 / 筑底反转候选，再挑 Top 标的进入 `smh company --deep`
