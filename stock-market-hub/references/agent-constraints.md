...
8. ⚠️ **雪球兜底数据不可靠（硬约束）**：当 `fund_flow.flow_source == "xueqiu_intraday_fallback"` 时，说明东财盘中资金流不可用、数据已降级为雪球兜底口径：
   - 必须**显式标注**："当前资金流数据为雪球兜底口径（东财盘中不可用），数据不完全准确"
   - 雪球兜底数据**不得**作为 `add`（加仓）或 `buy`/`probe`（买入）的**唯一资金面依据**。必须同时满足以下之一才能给出 add/buy/probe：
     a. 有其他独立数据源交叉验证（如公告面/板块面/价格结构面信号足够强，不依赖资金流即可成立）
     b. 已在决策报告中如实标注资金流数据不可靠，并注明"需在盘中东财恢复后重新确认"
   - 系统侧 `spc analyze now` 已对雪球兜底数据自动施加 confidence -0.05 扣分，但不会硬阻断 add/buy——阻断应交由 LLM / 人判断
   - **严格禁止**：仅凭雪球兜底数据的 `cross_validation` 或 `regime` 标签就断言"主力共振流入 / 反转已确认"——因为雪球只有当日累计一个数字，无法产出可靠的多周期验证
...
7. ⚠️ **A+H 双地上市公司代码选择（硬纪律）**：当用户提到的股票在 A 股和港股同时上市（A+H），**必须根据用户原话判断查哪个市场**：
   - 用户说"XX 的 H 股"、"XXh"、"港股 XX"、"XX 港股" → **必须查港股代码**（HKxxxx），不得查 A 股代码
   - 用户只说"XX"未指明市场 → 可查 A 股代码，但应提示"该公司也在港股上市（HKxxxx），是否需要查看港股数据？"
   - 常见 A+H 公司的代码对照（部分列表，查之前应先确认）：
     - 恒瑞医药：A 600276 / HK 01276
     - 比亚迪：A 002594 / HK 01211
     - 中芯国际：A 688981 / HK 00981
     - 紫金矿业：A 601899 / HK 02899
     - 药明康德：A 603259 / HK 02359
     - 海螺水泥：A 600585 / HK 00914
   - **绝不允许**用户说"H 股"或"港股"时仍返回 A 股数据，这是方向性错误
